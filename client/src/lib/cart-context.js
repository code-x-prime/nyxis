...
export function CartProvider({ children }) {
  ...
  const fetchCart = async (deliveryPincode = null) => {
    if (!isAuthenticated) return;

    setLoading(true);
    try {
      const url = deliveryPincode ? `/cart?deliveryPincode=${deliveryPincode}` : "/cart";
  ...
  // Update shipping options based on delivery pincode
  const updateShippingOptions = async (deliveryPincode) => {
    if (!isAuthenticated || !deliveryPincode) return;

    try {
      const res = await fetchApi(`/cart?deliveryPincode=${deliveryPincode}`, {
        credentials: "include",
      });
      if (res.data) {
        setCart(prevCart => ({
          ...
          selectedShippingOption: res.data.selectedShippingOption,
          freeShippingThreshold: res.data.freeShippingThreshold || 0,
          shippingMessage: res.data.shippingMessage || "",
          grandTotal: res.data.grandTotal || 0,
        }));
      }
    } catch (err) {
      console.error("Error updating shipping options:", err);
    }
  };
  ...
  const selectShippingOption = (option) => {
    setCart(prevCart => ({
      ...
      shippingTotal: option ? option.rate : 0,
      grandTotal: prevCart.subtotal + (option ? option.rate : 0),
      // grandTotal doesn't subtract coupon here — use getCartTotals() for accurate display
    }));
  };
